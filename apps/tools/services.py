...
def refresh_sales_metrics_for_pyme(pyme: Pyme) -> None:
    ...
    product_orders = list(
        ProductOrder.objects.filter(product__pyme=pyme).select_related(
            "order",
            "product",
            "product__pyme__category",
        )
    )
...
def sync_product_view_metric(product: Product) -> None:
    total_views = sum(
        Product.objects.filter(pyme=product.pyme, name=product.name).values_list(
            "get_requests_count",
            flat=True,
        )
    )
